class SocialUser < ActiveRecord::Base
  ...
  def self.find_for_facebook_oauth access_token, user_adding

    if user = SocialUser.where(:url => access_token.info.urls.Facebook).first
      user
    else
      if user_adding.nil? || user_adding.blank?
        new_user = User.create!(:name => access_token.extra.raw_info.name)
      else
        new_user = user_adding
      end
  ...
  def self.find_for_vkontakte_oauth access_token, user_adding
  ...
  def self.find_for_twitter_oauth access_token, user_adding
  ...
  def self.find_for_google_oauth access_token, user_adding
